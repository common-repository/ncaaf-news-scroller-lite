<?php
/*
Plugin Name: NCAAF News Scroller Lite
Description: Provides the Latest NCAAFSL Headlines, updated every 3-4 hours
Author: A93D
Version: 0.8.1
...
function NCAAFSLPageURL() {
 $NCAAFSLpageURL = 'http';
 $NCAAFSLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NCAAFSLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NCAAFSLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NCAAFSLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_ncaafslstats() 
{
?>
<h2>NCAAF Latest News Lite</h2>
<?php ncaafsl_stats(); ?>
<?php
}

function ncaafslstats_install()
{
register_sidebar_widget(__('NCAAF News Scroll Lite'), 'widget_ncaafslstats'); 
}
add_action("plugins_loaded", "ncaafslstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'ncaafsl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'ncaafsl_stats_remove' );

function ncaafsl_stats_install() 
{
...
add_option("ncaafsl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("ncaafsl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("ncaafsl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NCAAF SCROLLER Installation", "Hi\n\nLITE NCAAF SCROLLER Activated at \n\n".NCAAFSLPageURL()."\n\nLITE NCAAF SCROLLER Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function ncaafsl_stats_remove() 
{
/* Deletes the database field */
delete_option('ncaafsl_scroll_text_color');
delete_option('ncaafsl_scroll_text_color1');
delete_option('ncaafsl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'ncaafsl_stats_admin_menu');

function ncaafsl_stats_admin_menu() {

add_options_page('NCAAF Scroller Lite', 'NCAAF Scroller Lite Settings', 'administrator', 'ncaaf-news-scroller-lite.php', 'ncaafsl_stats_plugin_page');
}
}
function ncaafsl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('ncaafsl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('ncaafsl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('ncaafsl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/ncaaf_news_scroll_lite/ncaaf_news_scroll_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = ncaafsl_fetch_rss( $url );
...
function ncaafsl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>NCAAF Scroller LiteOptions Page</h2>
...
<!-- Start Advanced Plugins List -->
  ...
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NCAAF News Scroller package') ?></h5>
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/ncaaf-news-scroller.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/ncaaf-news-scroller.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
